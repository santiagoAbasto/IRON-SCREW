...
<section class="panel">
 ...
 <div class="data-table product-table">
  ...
  <div class="tr"><span>{{ $product->code }}</span><span>{{ $product->description }}</span><span>{{ $product->units_fractioned > 0 ? $fractionedDisplay : '—' }}</span><span>{{ $usesExactOrder ? 'A pedido' : ($product->units_bulk > 0 ? $bulkDisplay : '—') }}</span>
  ...
  <dialog class="form-dialog label-dialog" id="product-label-dialog-{{ $product->id }}" data-label-dialog data-standalone="true" data-code="{{ $product->code }}" data-description="{{ $product->description }}" data-quantity="0" data-fractioned="{{ $product->units_fractioned }}" data-bulk="{{ $product->units_bulk }}" data-unit-label="{{ $product->labelUnitText() }}" data-customer="" data-order="" data-logo="{{ asset('assets/figma/label-logo-bw.jpg') }}">
   ...
   @if($usesExactOrder)<div class="quantity-alert"><strong>Producto configurado a pedido.</strong><br>Cuando Granel está en cero, la etiqueta toma automáticamente la cantidad solicitada por el cliente. Para imprimir desde Productos, ingresá las unidades manualmente.</div>@elseif(!$product->units_fractioned)<div class="quantity-alert"><strong>Presentación fraccionada sin definir.</strong><br>El valor puede completarse más adelante; para esta impresión podés ingresar las unidades manualmente.</div>@endif
   ...
   <div class="form-grid"><label>Tipo de etiqueta<select data-label-type><option value="bulk">Granel</option><option value="fractioned">Fraccionado{{ $product->units_fractioned?' ('.number_format($product->units_fractioned,0,',','.').')':'' }}</option></select></label><label>Tamaño de etiqueta<select data-label-size><option value="80x50" selected>80 × 50 mm</option><option value="100x80">100 × 80 mm</option></select></label><label>{{ $product->label_unit==='kg'?'Kilogramos':'Unidades' }} por etiqueta<input type="number" min="{{ $product->label_unit==='kg'?'0.001':'1' }}" step="{{ $product->label_unit==='kg'?'0.001':'1' }}" placeholder="Ingresar cantidad" data-units-per-label></label><label>Total de cajas / etiquetas<input type="number" min="1" value="1" data-label-count></label></div>
   <div class="quantity-alert" data-quantity-alert hidden></div><p class="label-help" data-label-help></p>
   <div class="label-preview" data-label-preview><div class="thermal-label no-customer"><div class="thermal-product"><strong>{{ preg_replace('/(\d+)\s+[xX]\s+(\d+)/u', '$1 X $2', $product->description) }}</strong><span>{{ $product->code }}</span><b data-preview-units>— {{ $product->labelUnitText() }}</b></div><div class="thermal-brand"><img src="{{ asset('assets/figma/label-logo-bw.jpg') }}" alt="Iron Screw"><em data-preview-type>GRANEL</em></div></div></div>
