...
<section class="panel">
 ...
 <div class="data-table product-table">
  ...
  <div class="tr"><span>{{ $product->code }}</span><span>{{ $product->description }}</span><span>{{ $product->units_fractioned > 0 ? number_format($product->units_fractioned,0,',','.') : '—' }}</span><span>{{ $product->label_exact_order ? 'A pedido' : ($product->units_bulk > 0 ? number_format($product->units_bulk,0,',','.') : '—') }}</span>
  ...
  <dialog class="form-dialog label-dialog" id="product-label-dialog-{{ $product->id }}" data-label-dialog data-standalone="true" data-code="{{ $product->code }}" data-description="{{ $product->description }}" data-quantity="0" data-fractioned="{{ (int)$product->units_fractioned }}" data-bulk="{{ (int)$product->units_bulk }}" data-customer="" data-order="" data-logo="{{ asset('assets/figma/label-logo-bw.jpg') }}">
   ...
   @if($product->label_exact_order)<div class="quantity-alert"><strong>Producto configurado a pedido.</strong><br>En una orden, la etiqueta tomará automáticamente la cantidad solicitada por el cliente. Para imprimir desde Productos, ingresá las unidades manualmente.</div>@elseif(!$product->units_bulk||!$product->units_fractioned)<div class="quantity-alert"><strong>Presentación sin definir.</strong><br>Los valores en cero pueden completarse más adelante; para esta impresión podés ingresar las unidades manualmente.</div>@endif
   <div class="label-summary two"><span>Fraccionado <b>{{ $product->units_fractioned?:'—' }}</b></span><span>Granel <b>{{ $product->label_exact_order ? 'A pedido' : ($product->units_bulk > 0 ? number_format($product->units_bulk,0,',','.') : '—') }}</b></span></div>
