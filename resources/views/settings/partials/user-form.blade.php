<div class="form-grid">
 ...
 <label>Contraseña {{ $user?'(opcional)':'' }}<span class="password-field"><input id="user-password-{{ $user?->id ?? 'new' }}" type="password" name="password" {{ $user?'':'required' }} minlength="6" autocomplete="new-password"><button class="password-toggle" type="button" data-password-toggle="user-password-{{ $user?->id ?? 'new' }}" aria-label="Mostrar contraseña" aria-pressed="false">Mostrar</button></span><small>Mínimo 6 caracteres.</small></label>
