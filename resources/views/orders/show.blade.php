...
@php
    ...
    $unitLabel = $product?->labelUnitText() ?? 'UNIDADES';
    $quantityText = rtrim(rtrim(number_format($quantity,3,',','.'),'0'),',');
...
@endphp
<div class="tr" data-item-row="{{ $item->id }}"><span>{{ $item->code ?: 'S/C' }}</span><span>{{ $item->description }}</span><span data-item-quantity="{{ $item->id }}" class="{{ $savedAdjustment ? 'quantity-adjusted' : ($quantityMismatch ? 'quantity-mismatch' : ($quantityReview ? 'quantity-review' : '')) }}" @if($savedAdjustment) title="Ajustado por {{ $savedAdjustment['adjustedBy'] ?: 'un usuario' }}" @elseif($quantityMismatch) title="La cantidad pedida no coincide con una caja completa de granel ni de fraccionado" @elseif($quantityReview) title="Cierra con fraccionado, pero también cabe en una caja granel. Revisá la presentación." @endif>{{ $unitLabel==='KG'?$quantityText:number_format($quantity,0,',','.') }} @if($unitLabel==='KG')<small>KG</small>@endif</span>
...
<dialog class="form-dialog label-dialog" id="label-dialog-{{ $item->id }}" data-label-dialog data-item-id="{{ $item->id }}" data-concept-id="{{ $item->contabilium_concept_id }}" data-line-index="{{ $loop->index }}" data-code="{{ $item->code }}" data-description="{{ $item->description }}" data-quantity="{{ $quantity }}" data-fractioned="{{ $fractioned }}" data-bulk="{{ $bulk }}" data-unit-label="{{ $unitLabel }}" data-exact-order="{{ $exactOrder ? 'true' : 'false' }}" data-save-url="{{ route('orders.items.label-adjustment',[$order,$item]) }}" @if($savedAdjustment) data-saved-adjustment="{{ json_encode($savedAdjustment) }}" @endif data-customer="{{ $order->customer }}" data-order="{{ $order->number }}" data-logo="{{ asset('assets/figma/label-logo-bw.jpg') }}">
 ...
 <div class="label-summary"><span>Pedido <b>{{ $quantityText }} {{ $unitLabel }}</b></span><span>Fraccionado <b>{{ $fractioned?:'—' }}</b></span><span>Granel <b>{{ $exactOrder?'A pedido':($bulk?:'—') }}</b></span></div>
 <div class="form-grid"><label>Tipo de etiqueta<select data-label-type><option value="order">Pedido (cantidad exacta)</option><option value="bulk">Granel</option><option value="fractioned">Fraccionado{{ $fractioned?' ('.number_format($fractioned,0,',','.').')':'' }}</option></select></label><label>Tamaño de etiqueta<select data-label-size><option value="80x50" selected>80 × 50 mm</option><option value="100x80">100 × 80 mm</option></select></label><label>{{ $unitLabel==='KG'?'Kilogramos':'Unidades' }} por etiqueta<input type="number" min="{{ $unitLabel==='KG'?'0.001':'1' }}" step="{{ $unitLabel==='KG'?'0.001':'1' }}" placeholder="Ingresar cantidad" data-units-per-label></label><label>Total de cajas / etiquetas<input type="number" min="1" data-label-count></label></div>
 <div class="quantity-alert" data-quantity-alert hidden></div><p class="label-help" data-label-help></p>
 <div class="label-preview" data-label-preview><div class="thermal-label"><div class="thermal-customer {{ mb_strlen(trim((string) $order->customer)) > 23 ? 'customer-extra-long' : (mb_strlen(trim((string) $order->customer)) > 16 ? 'customer-long' : '') }}">{{ strtoupper($order->customer) }}</div><div class="thermal-product {{ mb_strlen(trim((string) $item->description)) > 58 ? 'description-extra-long' : (mb_strlen(trim((string) $item->description)) > 40 ? 'description-long' : '') }}"><strong>{{ $labelDescription }}</strong><span>{{ $item->code }}</span><b data-preview-units>— {{ $unitLabel }}</b></div><div class="thermal-brand"><img src="{{ asset('assets/figma/label-logo-bw.jpg') }}" alt="Iron Screw"><em data-preview-type>GRANEL</em></div></div></div>
